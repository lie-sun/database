                    ...
                    if (createtableresult) {
                        ...
                        if (deltableresult) {
                            ...
                            if (altertableresult) {
                                ...
                                var altertablethname = altertableresult[3]; //添加字段的名字
                                var addthstr = "<th>" + altertablethname + "</th>";
                                $("table." + altertablename + " thead tr").append(addthstr);
                            } else {
                                // 修改表之删除
                                var altertabledelreg = /^\s*alter\s+table\s+(\w+)\s+\n*drop\s+COLUMN\s+(\w+)/i;
                                ...
                                var alterdeltablename = altertabledelresult[1];
                                var altertabledelthname = altertabledelresult[2];
                                $("table."+alterdeltablename)
                            }
                        }
                    }
                }
